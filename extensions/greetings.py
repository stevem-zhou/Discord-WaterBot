...
async def starting_greet(event: hikari.PresenceUpdateEvent):
    if (event.user_id != None): #None will be replaced by a targeted user_id
        return
    ...
    dm_channel = await greeting_plugin.bot.rest.create_dm_channel(None) #None will be replaced by a targeted user_id
